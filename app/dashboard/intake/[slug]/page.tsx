...
export default function IntakeEditPage() {
  ...
  const deleteIntakeForm = useMutation(api.functions.intakeForms.deleteIntakeFormById);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const ok = await deleteIntakeForm({ id: slug as Id<"intakeForms"> });
      if (ok) router.push("/dashboard/intake");
    } finally {
      setDeleting(false);
    }
  };
      ...
      <div className="mb-6 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={deleting}
            className="inline-flex items-center justify-center rounded-md px-3 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 focus:ring-offset-[#0b1217] disabled:opacity-60"
          >
