...
export default function TicketsTableClient() {
  ...
  const assigneeIds = React.useMemo(
    () => (docs ? Array.from(new Set(docs.flatMap((d) => d.assignees))) : []),
    [docs],
  );

  const profiles = useQuery(api.functions.profiles.getProfilesByUserIds, {
    userIds: assigneeIds as Id<"users">[],
  });
              ...
              <tbody className="divide-y divide-slate-800">
                {rows?.map((r: TicketRow) => (
                  <tr key={r._id} className="hover:bg-slate-900/30">
                    ...
                    <td className="px-3 py-2 text-sm hidden md:table-cell">
                      <Assignees
                        names={r.assignees.map((id) =>
                          assignedUsers[id]?.fullName ?? id
                        )}
                      />
                    </td>
                    <td className="px-3 py-2 text-sm hidden md:table-cell text-slate-200 whitespace-nowrap">
