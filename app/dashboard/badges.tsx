...
export function NuetralBadge({ text }: { text: string }) {
  return (
    <span
      className={`inline-flex cursor-pointer items-center rounded-md px-2 py-0.5 text-xs bg-slate-700/40 text-slate-300 ring-1 ring-slate-600/40 focus:outline-none focus:ring-2 focus:ring-current`}
    >
      {text}
    </span>
  );
...
export function StatusBadge({ status }: { status: TicketRow["status"] }) {
  ...
  return (
    <span
      className={`inline-flex cursor-pointer items-center rounded-md px-2 py-0.5 text-xs ${
...
export function SeverityBadge(
  { severity }: { severity: TicketRow["severity"] },
) {
  const map: Record<string, string> = {
...
export function PriorityBadge(
  { priority }: { priority: TicketRow["priority"] },
) {
  const map: Record<string, string> = {
